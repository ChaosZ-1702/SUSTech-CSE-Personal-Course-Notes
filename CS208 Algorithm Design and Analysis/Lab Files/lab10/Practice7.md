## Practice 7 Chessboard Coverage Problem Use Divide-and-Conquer
...
public class lab10 {  // x is col, y is row
...
When $k > 0$, a $2^k × 2^k$ chessboard can be divided into four $2^{k-1} × 2^{k-1}$ sub-chessboards.

After dividing, since the original chessboard has only one missing square, only one of the four sub-chessboards contains the missing square, while the other three sub-chessboards have no missing squares.

To transform these three sub-chessboards without missing squares into those with missing squares, an L-shaped domino can be placed to cover the junction of the three sub-chessboards, and the three squares of the domino can be considered as the missing squares of these three sub-chessboards.

Apply the division recursively until the size of sub-chessboard is $1×1$.

#### Complexity Analysis

Let $T(k)$ denote the time required to cover a $2^k \times 2^k$ chessboard. $T(k)$ satisfies $T(k) = 4T(k-1) + O(1)$.

Therefore, the time complexity is $O(4^k)$.